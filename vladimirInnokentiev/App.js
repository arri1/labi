...
import { StyleSheet, TouchableOpacity, View, Dimensions, StatusBar, Animated } from "react-native";
import dayjs from 'dayjs'
import { transform } from "@babel/core";

const {width} = Dimensions.get('screen');
const SIZE = width * 0.9;
const TICK_INVERAL = 1000;
...
export default class App extends React.Component{
  ...
  _animate = () => {
    ...
    Animated.parallel([
      Animated.stagger(TICK_INVERAL / this.state.scales.length, scaleStaggerAnimations),
      Animated.timing(this.state.index, {
        toValue: this.state.tick,
        duration: TICK_INVERAL / 2,
        useNativeDriver: true
      })
    ]).start()
  };
  ...
  render() {
    const{index, scales: [smallQuadranScale, mediumQuadranScale, bigQuadranScale, secondsScale, minutesScale, hoursScale]} = this.state;
